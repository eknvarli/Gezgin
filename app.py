...
def view_results():
    ...
    cursor.execute(f'SELECT * FROM ilanlar WHERE sehir="{city}" AND yetenek="{lang}"')
    datas = cursor.fetchall()
...
def add():
    ...
    cursor = connection.cursor()
    cursor.execute('CREATE TABLE IF NOT EXISTS basvuru(ad TEXT, soyad TEXT, yetenek TEXT, aciklama TEXT, sehir TEXT, iletisim TEXT, ucret TEXT)')
    if request.method == 'POST':
        ad = request.form.get('ad')
        soyad = request.form.get('soyad')
        yetenek = request.form.get('skill')
        aciklama = request.form.get('aciklama')
        sehir = request.form.get('sehir')
        iletisim = request.form.get('iletisim')
        ucret = request.form.get('ucret')

        cursor.execute(f'INSERT INTO basvuru VALUES("{ad}","{soyad}","{yetenek}","{aciklama}","{sehir}","{iletisim}","{ucret}")')
        connection.commit()
        connection.close()

        return 'Your ad has been forwarded to the management team for approval.'
    else:
        return 'Post Error!'
...
def access():
    connection = sqlite3.connect('./databases/ilanlar.db')
    cursor = connection.cursor()
    cursor.execute('CREATE TABLE IF NOT EXISTS ilanlar(ad TEXT, soyad TEXT, yetenek TEXT, aciklama TEXT, sehir TEXT, iletisim TEXT, ucret TEXT)')
    if request.method == 'POST':
        ...
        cursor.execute(f'INSERT INTO ilanlar VALUES("{ad}","{soyad}","{yetenek}","{aciklama}","{sehir}","{iletisim}","{ucret}")')
        connection.commit()
        connection.close()
